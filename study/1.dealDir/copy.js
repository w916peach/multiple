...
const copy = (dir, target) => {
  ...
  if (stat.isFile()) { //如果需要复制的资源是一个文件，读取文件并写入target中
    fs.writeFileSync(path.join(target, path.basename(dir)), fs.readFileSync(dir));
    return;
  }
  if (stat.isDirectory()) { //如果需要复制的资源是一个目录，对内部目录复制，再执行一次copy方法
